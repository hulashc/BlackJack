...
def displayCards(cards):
    rows = [''] * 4  # I'm using a list of three empty strings to store the rows of text for the cards.
...
def getMove(playerHand, money):
    ...
    while True:
        ...
        movePrompt = ', '.join(['({}){}'.format(move, move[0]) for move in moves]) + '> '
        move = input(movePrompt).upper()

        if move and move[0] in moves:
            return move[0]
        else:
